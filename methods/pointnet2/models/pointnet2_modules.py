...
NPOINTS = [2048, 1024, 256, 16]
RADIUS = [[0.02, 0.05], [0.05, 0.1], [0.2, 0.5], [1.0, 2.0]]
...
class Pointnet2MSG(nn.Module):
    ...
    def forward(self, pointcloud: torch.cuda.FloatTensor):
        ...
        pointwise_feat = l_features[0]
        global_feat = torch.max(pointwise_feat, -1, keepdim=False)[0]
        return global_feat
...
if __name__ == "__main__":
    UT_POINTNET2 = True
    if UT_POINTNET2:
        model = Pointnet2MSG(input_channels=0).cuda()
        pc = torch.randn([2, 1024, 3]).cuda()
        print(model(pc).shape)
