...
class UTIOU(unittest.TestCase):
    ...
    def test_compute_iou_2d(self):
        import time
        from det3.ops import compute_iou_2d, write_pkl
        from det3.utils.utils import compute_iou
        for test_dtype in [np.float32, np.float64]:
            # npy
            boxes = np.array([[3, 2, 4, 4.5]])
            boxes = [boxes + np.abs(np.random.randn(4).reshape(1, 4)) for i in range(10)]
            ...
            gt = []
            for box_gt in boxes_gt:
                gt.append(compute_iou(box_gt.reshape(1, -1), others_gt, mode="2d-rot"))
            gt = np.stack(gt, axis=0)
            times = 100

            est = compute_iou_2d(boxes, others)
            self.assertTrue(est.shape == gt.shape)
            self.assertTrue(np.allclose(est, gt, atol=1e-1))
            self.assertTrue(est.dtype == boxes.dtype)
            t1 = time.time()
            for i in range(times):
                est = compute_iou_2d(boxes, others)
            t = (time.time()-t1) / times * 1000
            print(f"compute_iou_2d np: {t:.2f} ms")
            # torch
            boxes_ts = torch.from_numpy(boxes)
            others_ts = torch.from_numpy(others)
            est_ts = compute_iou_2d(boxes_ts, others_ts)
            self.assertTrue(est_ts.numpy().shape == gt.shape)
            self.assertTrue(np.allclose(est_ts.numpy(), gt, atol=1e-1))
            self.assertTrue(est_ts.dtype == boxes_ts.dtype)
            t1 = time.time()
            for i in range(times):
                est_ts = compute_iou_2d(boxes_ts, others_ts)
            t = (time.time()-t1) / times * 1000
            print(f"compute_iou_2d torch: {t:.2f} ms")
            # torch cuda
            boxes_tsgpu = boxes_ts.cuda()
            others_tsgpu = others_ts.cuda()
            est_tsgpu = compute_iou_2d(boxes_tsgpu, others_tsgpu)
            self.assertTrue(est_tsgpu.cpu().numpy().shape == gt.shape)
            self.assertTrue(np.allclose(est_tsgpu.cpu().numpy(), gt, atol=1e-1))
            self.assertTrue(est_tsgpu.dtype == boxes_tsgpu.dtype)
            torch.cuda.synchronize()
            ...
            torch.cuda.synchronize()
            t = (time.time()-t1) / times * 1000
            print(f"compute_iou_2d torch cuda: {t:.2f} ms")
    ...
    def test_compute_iou_2drot(self):
        import time
        from det3.ops import compute_iou_2drot, write_pkl
        from det3.utils.utils import compute_iou
        for test_dtype in [np.float32, np.float64]:
            # npy
            boxes = np.array([[3, 2, 4, 4.5, 0.1]])
            ...
            gt = np.stack(gt, axis=0)
            times = 100
            est = compute_iou_2drot(boxes, others)
            self.assertTrue(est.shape == gt.shape)
            self.assertTrue(np.allclose(est, gt, atol=1e-1))
            self.assertTrue(est.dtype == boxes.dtype)
            t1 = time.time()
            for i in range(times):
                est = compute_iou_2drot(boxes, others)
            t = (time.time()-t1) / times * 1000
            print(f"compute_iou_2drot np: {t:.2f} ms")
            # torch
            boxes_ts = torch.from_numpy(boxes)
            others_ts = torch.from_numpy(others)
            est_ts = compute_iou_2drot(boxes_ts, others_ts)
            self.assertTrue(est_ts.numpy().shape == gt.shape)
            self.assertTrue(np.allclose(est_ts.numpy(), gt, atol=1e-1))
            self.assertTrue(est_ts.dtype == boxes_ts.dtype)
            t1 = time.time()
            for i in range(times):
                est_ts = compute_iou_2drot(boxes_ts, others_ts)
            t = (time.time()-t1) / times * 1000
            print(f"compute_iou_2drot torch: {t:.2f} ms")
            # torch cuda
            boxes_tsgpu = torch.from_numpy(boxes).cuda()
            others_tsgpu = torch.from_numpy(others).cuda()
            est_tsgpu = compute_iou_2drot(boxes_tsgpu, others_tsgpu)
            self.assertTrue(est_tsgpu.cpu().numpy().shape == gt.shape)
            self.assertTrue(np.allclose(est_tsgpu.cpu().numpy(), gt, atol=1e-1))
            self.assertTrue(est_tsgpu.dtype == boxes_tsgpu.dtype)
            torch.cuda.synchronize()
            ...
            torch.cuda.synchronize()
            t = (time.time()-t1) / times * 1000
            print(f"compute_iou_2drot torch cuda: {t:.2f} ms")
    ...
    def test_compute_iou_3drot(self):
        import time
        from det3.ops import compute_iou_3drot, write_pkl
        from det3.utils.utils import compute_iou
        for test_dtype in [np.float32, np.float64]:
            # npy
            boxes = np.array([[5, 3.5, 1, 3, 5, 2, 0.05]])
            ...
            gt = []
            for box_gt in boxes:
                gt.append(compute_iou(box_gt.reshape(1, -1), others, mode="3d-rot"))
            gt = np.stack(gt, axis=0)
            times = 100
            est = compute_iou_3drot(boxes, others)
            self.assertTrue(est.shape == gt.shape)
            self.assertTrue(np.allclose(est, gt, atol=1e-1))
            self.assertTrue(est.dtype == boxes.dtype)
            t1 = time.time()
            for i in range(times):
                est = compute_iou_3drot(boxes, others)
            t = (time.time()-t1) / times * 1000
            print(f"compute_iou_3drot np: {t:.2f} ms")
            # torch
            boxes_ts = torch.from_numpy(boxes)
            others_ts = torch.from_numpy(others)
            est_ts = compute_iou_3drot(boxes_ts, others_ts)
            self.assertTrue(est_ts.numpy().shape == gt.shape)
            self.assertTrue(np.allclose(est_ts.numpy(), gt, atol=1e-1))
            self.assertTrue(est_ts.dtype == boxes_ts.dtype)
            t1 = time.time()
            for i in range(times):
                est_ts = compute_iou_3drot(boxes_ts, others_ts)
            t = (time.time()-t1) / times * 1000
            print(f"compute_iou_3drot torch: {t:.2f} ms")
            # # torch cuda
            boxes_tsgpu = torch.from_numpy(boxes).cuda()
            others_tsgpu = torch.from_numpy(others).cuda()
            est_tsgpu = compute_iou_3drot(boxes_tsgpu, others_tsgpu)
            self.assertTrue(est_tsgpu.cpu().numpy().shape == gt.shape)
            self.assertTrue(np.allclose(est_tsgpu.cpu().numpy(), gt, atol=1e-1))
            self.assertTrue(est_tsgpu.dtype == boxes_tsgpu.dtype)
            torch.cuda.synchronize()
            ...
            torch.cuda.synchronize()
            t = (time.time()-t1) / times * 1000
            print(f"compute_iou_3drot torch cuda: {t:.2f} ms")
